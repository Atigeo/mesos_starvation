...
struct Client
{
  Client(const std::string& _name, const std::string& _role, double _share, uint64_t _allocations)
    : name(_name), role(_role), share(_share), allocations(_allocations) {}
...
public:
  ...
  virtual void add(const std::string& name, const std::string& role, double weight = 1);

  virtual void remove(const std::string& name);

  virtual void activate(const std::string& name, const std::string& role);
  ...
  virtual int count();
  virtual void updateAllocations();
