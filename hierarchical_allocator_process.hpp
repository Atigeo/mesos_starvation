...
struct Slave {
	...
	explicit Slave(const SlaveInfo& _info) :
			available(_info.resources()), connected(true), whitelisted(false), checkpoint(
					_info.checkpoint()), info(_info) {
	}
...
struct Framework {
	...
	explicit Framework(const FrameworkInfo& _info) :
			checkpoint(_info.checkpoint()), info(_info) {
	}
...
public:
	...
	void initialize(const Flags& flags, const process::PID<Master>& _master,
			const hashmap<std::string, RoleInfo>& _roles);

	void frameworkAdded(const FrameworkID& frameworkId,
			const FrameworkInfo& frameworkInfo, const Resources& used);
	...
	void slaveAdded(const SlaveID& slaveId, const SlaveInfo& slaveInfo,
			const hashmap<FrameworkID, Resources>& used);

	void slaveRemoved(const SlaveID& slaveId);

	void slaveDisconnected(const SlaveID& slaveId);

	void slaveReconnected(const SlaveID& slaveId);
	...
	void resourcesUnused(const FrameworkID& frameworkId, const SlaveID& slaveId,
			const Resources& resources, const Option<Filters>& filters);
...
protected:
	...
	// on this slave.
	bool isFiltered(const FrameworkID& frameworkId, const SlaveID& slaveId,
...
public:
	RefusedFilter(const SlaveID& _slaveId, const Resources& _resources,
	...
	virtual bool filter(const SlaveID& slaveId, const Resources& resources) {
		return slaveId == this->slaveId && resources <= this->resources && // Refused resources are superset.
...
:
		ProcessBase(process::ID::generate("hierarchical-allocator")), initialized(
				false) {
}
...
template<class RoleSorter, class FrameworkSorter>
process::PID<HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter> > HierarchicalAllocatorProcess<
		RoleSorter, FrameworkSorter>::self() {
	return process::PID<
			HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter> >(this); // NOLINT(whitespace/line_length)
}

template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::initialize(
		const Flags& _flags, const process::PID<Master>& _master,
		const hashmap<std::string, RoleInfo>& _roles) {
	flags = _flags;
	master = _master;
	roles = _roles;
	...
	{
		// since the Client struct receives for the moment the role we need a
		// trick to avoid an error when checking if the client role != *.
		roleSorter->add(name, name, roleInfo.weight());
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::frameworkAdded(
	...
	sorters[role]->add(frameworkId.value(), role);
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::frameworkRemoved(
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::frameworkActivated(
		const FrameworkID& frameworkId, const FrameworkInfo& frameworkInfo) {
	CHECK(initialized);

	const std::string& role = frameworkInfo.role();
	sorters[role]->activate(frameworkId.value(), role);
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::frameworkDeactivated(
		const FrameworkID& frameworkId) {
	CHECK(initialized);
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::slaveAdded(
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::slaveRemoved(
		const SlaveID& slaveId) {
	CHECK(initialized);
	CHECK(slaves.contains(slaveId));
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::slaveDisconnected(
		const SlaveID& slaveId) {
	CHECK(initialized);
	CHECK(slaves.contains(slaveId));
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::slaveReconnected(
		const SlaveID& slaveId) {
	CHECK(initialized);
	CHECK(slaves.contains(slaveId));
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::updateWhitelist(
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::resourcesRequested(
	const FrameworkID& frameworkId, const std::vector<Request>& requests) {
CHECK(initialized);
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::resourcesUnused(
...
LOG(INFO) << "resourcesUnunsed method: allocatable size: "
		<< resources.allocatable().size();
VLOG(1) << "Framework " << frameworkId << " left " << resources.allocatable()
...
if (seconds != Duration::zero()) {
	LOG(INFO) << "Framework " << frameworkId << " filtered slave " << slaveId
			<< " for " << seconds;

	// Create a new filter and delay it's expiration.
	Filter* filter = new RefusedFilter(slaveId, resources,
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::resourcesRecovered(
	const FrameworkID& frameworkId, const SlaveID& slaveId,
	const Resources& resources) {
CHECK(initialized);
...
LOG(INFO) << "resourcesRecovered method: allocatable size: "
		<< resources.allocatable().size();
...
if (frameworks.contains(frameworkId)
		&& sorters[frameworks[frameworkId].role()]->contains(
				frameworkId.value())) {
	const std::string& role = frameworks[frameworkId].role();
	sorters[role]->unallocated(frameworkId.value(), resources);
	sorters[role]->remove(resources);
...
if (slaves.contains(slaveId)) {
	...
	LOG(INFO) << "Recovered " << resources.allocatable()
			...
			<< ") on slave " << slaveId << " from framework " << frameworkId;
}
}

template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::offersRevived(
	const FrameworkID& frameworkId) {
CHECK(initialized);
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::batch() {
CHECK(initialized);
allocate();
delay(flags.allocation_interval, self(), &Self::batch);
}
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::allocate() {
CHECK(initialized);
...
foreach (const std::string& frameworkIdValue, sorters[role]->sort()) {
	...
	foreach (const SlaveID& slaveId, slaveIds) {
		...
		LOG(INFO) << "allocate method: on frameworkID:" << frameworkIdValue << " Filtered? " << filtered;

		bool connectedSlave =slaves[slaveId].connected;
		LOG(INFO) << "allocate method: on frameworkID:" << frameworkIdValue << " connectedSlave? " << connectedSlave;

		bool whiteListedFramework = slaves[slaveId].whitelisted;
		LOG(INFO) << "allocate method: on frameworkID:" << frameworkIdValue << " whiteListedFramework? " << whiteListedFramework;

		bool allocatableResources = allocatable(resources);
		LOG(INFO) << "allocate method: on frameworkID:" << frameworkIdValue << " allocateableResources? " << allocatableResources;

		if (!filtered &&
				connectedSlave &&
				whiteListedFramework &&
	...
	if (!offerable.empty()) {
		...
		LOG(INFO) << "allocate method: on frameworkID: " << frameworkId << " : dispatch call";
...
template<class RoleSorter, class FrameworkSorter>
void HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::expire(
const FrameworkID& frameworkId, Filter* filter) {
...
template<class RoleSorter, class FrameworkSorter>
bool HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::isWhitelisted(
const SlaveID& slaveId) {
CHECK(initialized);
...
VLOG(1) << "Filtered " << resources << " on non-checkpointing slave " << slaveId
...
if (filter->filter(slaveId, resources)) {
	VLOG(1) << "Filtered " << resources << " on slave " << slaveId
...
template<class RoleSorter, class FrameworkSorter>
bool HierarchicalAllocatorProcess<RoleSorter, FrameworkSorter>::allocatable(
const Resources& resources) {
